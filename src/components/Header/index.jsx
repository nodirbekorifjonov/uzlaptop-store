...
const Header = () => {
  ...
  const handleChangeCurrency = () => {
    !openCurrency ? setOpenCurrency(true) : setOpenCurrency(false);
    setOpenLanguage(false);
  };
  // HandleChangeLang
  const handleChangeLang = () => {
    !openLanguage ? setOpenLanguage(true) : setOpenLanguage(false);
    setOpenCurrency(false);
  };

  return (
    <header id="header" className="w-full bg-[#fff]">
      {/* First Header */}
      <div className="header-first border-b-[1px] border-[#E0E0E0] py-[2rem] max-[876px]:py-[1.5rem] max-[876px]:border-transparent">
        <div className="site-container flex justify-between items-center">
          ...
          {/* Mobile Nav */}
          <nav
            className={`mobile-nav opacity-0 invisible py-[2rem] transition-all duration-300 absolute bg-white left-0 top-[13rem] w-[80%] h-full ${addActive}`}
          >
            <ul className="mobile-nav__list flex flex-col mb-[3rem]">
              <li className="mobile-nav__item">
                <Link
                  onClick={() => setAddActive("")}
                  to="/"
                  className={`mobile-nav__item bg-white border-b-[1px] py-[1.5rem] px-[2rem] flex items-center gap-x-[1rem] text-[#676f7a] translate-x-[-20px] invisible opacity-0 transition-all duration-[2s] ${addActive} hover:bg-[#E5F1FF] hover:transition-none hover:duration-0`}
                >
                  <FaUser />
                  <span className="text-[1.6rem]">Profile</span>
                </Link>
              </li>
              <li className="mobile-nav__item">
                <Link
              ...
              <li className="mobile-nav__item">
                ...
                >
                  <FaCartShopping />
                  <span className="text-[1.6rem]">My Cart</span>
                </Link>
              </li>
              <li className="mobile-nav__item">
                <Link
              ...
              <li className="mobile-nav__item">
                ...
                >
                  <FaPhoneAlt />
                  <span className="text-[1.6rem]">Contact</span>
                </Link>
              </li>
              <li className="mobile-nav__item">
                <Link
                ...
                >
                  <FaQuestion />
                  <span className="text-[1.6rem]">FaQ</span>
                </Link>
              </li>
            </ul>
            <ul
              className={`header-second__list select-none flex items-center gap-x-[3rem] justify-center text-[#555b63] translate-y-0-[-20px] invisible opacity-0 transition-all duration-[4s] ${addActive}`}
            >
              {/* Currency */}
              <li
              ...
              >
                ...
                {openCurrency && (
                  <ul className="absolute w-[12rem] py-[1rem] rounded-[1rem] shadow-[10px_6px_37px_-1px_rgba(0,0,0,0.04)] bg-white top-[3.5rem] border border-[#e0e0e098] right-0 flex flex-col justify-center">
                    <li
      ...
      <div
        onClick={handleAddActive}
        className={`overlay absolute top-0 left-0 bg-black opacity-0 w-full h-full block z-[-2] transition-all duration-300 ${addActive}`}
      ></div>
    </header>
  );
};
